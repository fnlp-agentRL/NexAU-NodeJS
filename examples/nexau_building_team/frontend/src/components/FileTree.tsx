...
const FolderOpenIcon = () => (
  <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
    <path
      d="M2 4C2 3.44772 2.44772 3 3 3H6.5L8 5H13C13.5523 5 14 5.44772 14 6V6H3.5L2 12V4Z"
      fill="#F5C542"
    />
  ...
  return (
    <>
      <div
        ...
        onClick={() => (node.isDirectory ? onToggle(node) : onSelectFile(node.path))}
      >
        <span style={ns.icon}>
          {node.isDirectory ? (
            <>
              ...
              <span style={{ marginLeft: 2 }}>
                {node.expanded ? <FolderOpenIcon /> : <FolderClosedIcon />}
              </span>
            </>
          ) : (
        ...
        <span
          style={{
            ...ns.name,
            color: isSelected ? "#2D2A26" : node.isDirectory ? "#6B6560" : "#A09890",
          }}
        >
          {node.name}
        </span>
...
export function FileTree({ selectedPath, onSelectFile, isStreaming, mockNodes }: FileTreeProps) {
  ...
  const refresh = useCallback(async () => {
    ...
    const refreshExpanded = async (nodes: FileNode[]): Promise<FileNode[]> => {
