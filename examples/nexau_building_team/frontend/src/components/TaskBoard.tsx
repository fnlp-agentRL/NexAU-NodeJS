...
export function TaskBoard({ userId, sessionId, isStreaming }: TaskBoardProps) {
  ...
  return (
    <div style={styles.board}>
      <div style={styles.titleRow} onClick={() => setCollapsed((c) => !c)}>
        <span style={styles.title}>Task Board</span>
        <span style={styles.collapseHint}>{collapsed ? `▸ ${tasks.length} tasks` : "▾"}</span>
      </div>
      {!collapsed ? (
        <div style={styles.list}>
          {visible.map((t) => (
            <div key={t.task_id} style={styles.task}>
              <span style={{ ...styles.badge, ...statusStyle(t.status) }}>{t.status}</span>
              <span style={styles.taskTitle}>{t.title}</span>
              {t.assignee_agent_id && <span style={styles.assignee}>→ {t.assignee_agent_id}</span>}
            </div>
          ))}
        </div>
      ) : (
        <div style={styles.list}>
          ...
          {hiddenCount > 0 && <div style={styles.moreHint}>+{hiddenCount} more…</div>}
        </div>
      )}
    </div>
  );
