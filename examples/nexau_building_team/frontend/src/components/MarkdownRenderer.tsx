...
function ZoomBar({
  ...
  return (
    <div style={mdStyles.zoomBar}>
      <button style={mdStyles.zoomBtn} onClick={onZoomOut}>
        −
      </button>
      <span style={mdStyles.zoomLabel}>{Math.round(zoom * 100)}%</span>
      <button style={mdStyles.zoomBtn} onClick={onZoomIn}>
        +
      </button>
      <button style={{ ...mdStyles.zoomBtn, marginLeft: 4, fontSize: 11 }} onClick={onReset}>
        Reset
      </button>
      {extra}
    </div>
