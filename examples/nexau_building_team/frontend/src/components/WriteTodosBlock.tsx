...
export function parseTodos(argsJson: string, partial = false): TodoItem[] | null {
  try {
      ...
      <div style={styles.list}>
        {todos.map((todo, i) => (
          <div key={i} style={styles.item}>
            <span style={statusIconStyle(todo.status)}>{statusIcon(todo.status)}</span>
            <span
              style={{
                ...styles.itemText,
            ...
            {todo.status === "in_progress" && <span style={styles.statusDot} />}
          </div>
        ))}
      </div>

      {result && <div style={styles.footer}>{allDone ? "All tasks tracked" : "Tasks updated"}</div>}
    </div>
  );
}
