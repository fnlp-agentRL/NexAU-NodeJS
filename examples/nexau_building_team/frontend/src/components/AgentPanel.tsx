...
export function AgentPanel({ agent, onSendMessage }: AgentPanelProps) {
  ...
  const historyAnsweredAskIds = useMemo(() => {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      if (block.kind === "tool_call" && block.name === "ask_user" && block.result !== null) {
    ...
    <div ref={containerRef} onScroll={handleScroll} style={styles.root}>
      {agent.blocks.map((block, i) => {
        switch (block.kind) {
          ...
          case "thinking":
            return <ThinkingBlock key={`th-${i}`} content={block.content} />;
          case "tool_call": {
...
function ThinkingBlock({ content }: { content: string }) {
  ...
  return (
    <div style={styles.thinkingBlock}>
      <div style={styles.thinkingHeader} onClick={() => setExpanded((v) => !v)}>
    ...
    <div style={styles.toolCall}>
      ...
      {expanded && (
        <div style={styles.toolBody}>
          ...
          <div style={styles.toolSection}>
            <div style={styles.outputHeader} onClick={() => setOutputExpanded((v) => !v)}>
            ...
            {outputExpanded && !result && (
              <div style={styles.metaLine}>
                {argsDone ? "Tool is running — output will appear here." : "Waiting for args…"}
              </div>
            )}

            {outputExpanded && result && (
              <>
                {returnDisplay && <div style={styles.returnDisplay}>{returnDisplay}</div>}
                {durationMs !== null && <div style={styles.metaLine}>duration: {durationMs}ms</div>}
                {errorMessage && <div style={styles.metaLine}>error: {errorMessage}</div>}
                <pre style={styles.codeBlock}>{outputContentText || "(empty)"}</pre>
              </>
            )}
          </div>
        </div>
