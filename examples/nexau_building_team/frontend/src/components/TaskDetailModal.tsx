import { X, Circle, Loader2, CheckCircle2, Clock, User, FileText } from "lucide-react";
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {task.dependencies.map((dep) => (
                  <span
                    key={dep}
                    className="text-[10px] font-mono bg-slate-100 border border-slate-200 px-2 py-0.5 text-slate-600"
                  >
                    {dep}
                  </span>
                ))}
          ...
          {!task.result_summary && task.status !== "completed" && (
            <p className="text-xs text-slate-400 italic">
              No deliverable yet — task is still{" "}
