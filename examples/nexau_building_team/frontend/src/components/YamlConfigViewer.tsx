...
export function YamlConfigViewer({ content }: YamlConfigViewerProps) {
  ...
  return configType === "agent" ? <AgentConfigView doc={doc} /> : <ToolConfigView doc={doc} />;
}
...
function Badge({ text, color }: { text: string; color: string }) {
  return <span style={{ ...s.badge, background: color }}>{text}</span>;
}
  ...
  return (
    <div style={s.root}>
          ...
          {skills.map((sk, i) => {
            ...
            return (
              <div key={i} style={s.listItem}>
                ...
                <span
                  style={{ color: "#A09890", fontSize: 11, marginLeft: 8, fontFamily: "monospace" }}
                >
                  {path}
                </span>
              </div>
      ...
      <Section title="Execution">
        ...
        {stopTools.length > 0 && <Field label="Stop Tools" value={stopTools.join(", ")} />}
      </Section>
      ...
      {middlewares.length > 0 && (
        <Section title={`Middlewares (${middlewares.length})`}>
          {middlewares.map((m, i) => (
            <div key={i} style={s.listItem}>
              <span style={{ color: "#A09890", fontSize: 12, fontFamily: "monospace" }}>
                {m.import}
              </span>
            </div>
          ))}
