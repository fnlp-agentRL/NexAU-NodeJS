...
export const MOCK_TASKS: Record<Phase, TaskInfo[]> = {
  ...
  [Phase.DESIGN]: [
    ...
    makeTask({
      ...
      status: "in_progress",
      created_by: LEADER_ID,
      assignee_agent_id: RFC_WRITER_ID,
  ...
  [Phase.IMPLEMENTATION]: [
    ...
    makeTask({
      ...
      status: "in_progress",
      created_by: LEADER_ID,
      assignee_agent_id: BUILDER_DB_ID,
    ...
    makeTask({
      ...
      status: "in_progress",
      created_by: LEADER_ID,
      assignee_agent_id: BUILDER_SKILL_ID,
    ...
    makeTask({
      ...
      status: "in_progress",
      created_by: LEADER_ID,
      assignee_agent_id: BUILDER_CFG_ID,
...
export const IMPL_TASK_SNAPSHOTS: TaskInfo[][] = [
  ...
  [
    ...MOCK_TASKS[Phase.IMPLEMENTATION].map((t) => {
      if (t.task_id === "SKILL-001")
        return { ...t, status: "completed", result_summary: "enterprise_query skill 已完成" };
  ...
  [
    ...MOCK_TASKS[Phase.IMPLEMENTATION].map((t) => {
      if (t.task_id === "SKILL-001")
        return { ...t, status: "completed", result_summary: "enterprise_query skill 已完成" };
      if (t.task_id === "SKILL-002")
        return { ...t, status: "completed", result_summary: "business_analysis skill 已完成" };
      if (t.task_id === "SKILL-003") return { ...t, status: "in_progress", is_blocked: false };
      if (t.task_id === "CONFIG-001")
        return { ...t, status: "completed", result_summary: "nexau.yaml 配置已完成" };
  ...
  [
    ...MOCK_TASKS[Phase.IMPLEMENTATION].map((t) => {
      ...
      if (t.task_id === "SKILL-002")
        return { ...t, status: "completed", result_summary: "business_analysis skill 已完成" };
      if (t.task_id === "SKILL-003")
        return { ...t, status: "completed", result_summary: "policy_match skill 已完成" };
      if (t.task_id === "CONFIG-001")
        return { ...t, status: "completed", result_summary: "nexau.yaml 配置已完成" };
      if (t.task_id === "BUILD-001")
        return { ...t, status: "completed", result_summary: "run.py 已完成" };
      return t;
    }),
    makeTask({
      task_id: "TEST-001",
