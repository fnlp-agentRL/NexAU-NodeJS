    ...
    events.push(
      env(
        ...
        role,
        {
          type: "TOOL_CALL_ARGS",
          tool_call_id: tcId,
          delta: args.slice(i, i + argsChunkSize),
        },
        argsDelay,
      ),
    );
...
function buildDesignPhase(): TimedEnvelope[] {
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({
        content:
          "请为经信委数据分析智能体编写 RFC 设计文档。数据库类型：PostgreSQL。需要支持的分析能力：企业信息查询、经营数据分析、政策匹配、产业链分析。有数据字典可用。请详细设计每个 Skill 的输入输出和数据库查询逻辑。",
        ...
        questions: [
          {
            header: "设计方案审批",
            question:
              "RFC-0001 设计文档已完成，包含以下 **5 个 Skills** 的详细设计：\n\n" +
...
function buildImplementationPhase(): TimedEnvelope[] {
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({ role: "builder" }),
      JSON.stringify({
        agent_id: BUILDER_SKILL_ID,
        role_name: BUILDER_SKILL_ROLE,
        status: "running",
      }),
      { startDelay: 200 },
    ),
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({
        title: "智能体配置与启动脚本",
        description: "编写 nexau.yaml 和 run.py",
        priority: 1,
      }),
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({
        content:
          "SKILL-001: 请探索 PostgreSQL 数据库结构，实现企业信息查询 Skill。数据字典在 data/schema.sql。",
        to_agent_id: BUILDER_DB_ID,
      }),
      "Message sent",
      { startDelay: 300 },
    ),
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({
        content:
          "SKILL-002 + SKILL-003: 请实现经营分析和政策匹配两个 Skills。参考 rfcs/0001-jxw-agent.md。",
        to_agent_id: BUILDER_SKILL_ID,
      }),
      "Message sent",
      { startDelay: 100 },
    ),
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({
        content:
          "CONFIG-001 + BUILD-001: 请编写智能体 nexau.yaml 配置和 run.py 启动脚本。参考 RFC 中的 Skills 设计。",
        to_agent_id: BUILDER_CFG_ID,
      }),
      "Message sent",
      { startDelay: 100 },
    ),
...
function buildBuilderDbEvents(): TimedEnvelope[] {
  ...
  events.push(
    teamMessage(
      ...
      R,
      LEADER_ID,
      "SKILL-001: 请探索 PostgreSQL 数据库结构，实现企业信息查询 Skill。",
      200,
    ),
  );
  ...
  events.push(
    ...toolCall(
      ...
      "run_shell_command",
      JSON.stringify({
        command: 'psql -h localhost -U nexau -d jxw_db -c "\\\\d enterprise_info"',
      }),
  ...
  events.push(
    ...toolCall(
      ...
      "run_shell_command",
      JSON.stringify({
        command: 'psql -h localhost -U nexau -d jxw_db -c "SELECT count(*) FROM enterprise_info"',
      }),
      " count\n-------\n 12847\n(1 row)",
    ),
  );
...
function buildBuilderSkillEvents(): TimedEnvelope[] {
  ...
  events.push(
    teamMessage(
      ...
      R,
      LEADER_ID,
      "SKILL-002 + SKILL-003: 请实现经营分析和政策匹配两个 Skills。",
      200,
    ),
  );
...
function buildBuilderCfgEvents(): TimedEnvelope[] {
  ...
  events.push(
    teamMessage(
      ...
      R,
      LEADER_ID,
      "CONFIG-001 + BUILD-001: 请编写智能体 nexau.yaml 配置和 run.py 启动脚本。",
      200,
    ),
  );
...
function buildTestingEvents(): TimedEnvelope[] {
  ...
  events.push(
    teamMessage(A, R, LEADER_ID, "TEST-001: 请运行集成测试验证所有 Skills 正常工作。", 200),
  );
  ...
  events.push(
    ...toolCall(
      ...
      JSON.stringify({
        command:
          "python -c \"from nexau import Agent; a = Agent.from_config('nexau.yaml'); print('Config loaded:', a.name)\"",
      }),
