...
export default function ModernAgentDashboard() {
  ...
  const getScript = () => {
    ...
    s.push({
      ...
      action: () => {
        addMessage(
          "user",
          "leader",
          `初始化 NexAU Agent 项目。我们需要 ${builderCount} 个 SKILL 并行开发。`,
        );
        updateAgent("user", "working");
    ...
    s.push({
      ...
      action: () => {
        ...
        addMessage(
          "leader",
          "user",
          "收到指令。正在拆解 NexAU Agent 组件并组建 Swarm 团队...",
          "system",
        );
    ...
    s.push({
      ...
      action: () => {
        ...
        addMessage(
          "leader",
          "rfc",
          "Doc-Alpha, 请起草 Agent 的 System Prompt 并生成 Config Yaml。",
        );
        updateAgent("leader", "idle");
        updateAgent("rfc", "working");
    ...
    s.push({
      ...
      action: () => {
        ...
        updateAgent("rfc", "success");
        updateTaskStatus("t_rfc", "done");
    ...
    s.push({
      ...
      action: () => {
        ...
        addMessage(
          "leader",
          "tester",
          "模块已集结。执行全链路沙盒测试，生成 Tool Config。",
          "system",
        );
        updateAgent("leader", "idle");
    ...
    s.push({
      ...
      action: () => {
        ...
        addMessage(
          "tester",
          "b1",
          "❌ 失败: SKILL 01 返回体不符合要求，Tool Config 生成失败！",
          "error",
        );
        updateAgent("b1", "working");
    ...
    s.push({
      ...
      action: () => {
        ...
        addMessage(
          "leader",
          "user",
          "测试完成，正在封装 NexAU 启动脚本 (Launch Script)...",
          "system",
        );
      },
    });
    ...
    s.push({
      ...
      action: () => {
        ...
        addMessage(
          "leader",
          "user",
          "🎉 NexAU Agent 构建完成，完整 Artifact 已准备就绪！",
          "success",
        );
        setIsPlaying(false);
      },
    ...
    setMessages([
      {
        id: "init",
        fromId: "system",
        text: "System ready. Awaiting initialization...",
        type: "system",
      },
    ]);
    setStep(0);
  ...
  const layoutedAgents = useMemo(() => {
    ...
    workers.sort((a, b) => {
      const weight = (id) =>
        id === "rfc" ? 0 : id === "tester" ? 100 : parseInt(id.replace("b", ""));
  ...
  const completedArtifactParts =
    (artifactState.systemPrompt ? 1 : 0) +
    (artifactState.agentConfig ? 1 : 0) +
    (artifactState.toolConfig ? 1 : 0) +
    (artifactState.startupScript ? 1 : 0) +
    Object.keys(artifactState.skills).length;
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3 bg-white px-3 py-1.5 border border-slate-200 shadow-sm">
            ...
            <div className="flex items-center gap-1 bg-slate-50 border border-slate-200 px-1">
              <button
                onClick={() => setBuilderCount(Math.max(1, builderCount - 1))}
                disabled={isPlaying}
                className="px-1.5 text-slate-500 hover:text-slate-900 disabled:opacity-30"
              >
                -
              </button>
              <span className="text-sm font-mono w-4 text-center text-blue-600 font-bold">
                {builderCount}
              </span>
              <button
                onClick={() => setBuilderCount(Math.min(5, builderCount + 1))}
                disabled={isPlaying}
                className="px-1.5 text-slate-500 hover:text-slate-900 disabled:opacity-30"
              >
                +
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="w-72 border-r border-slate-200 bg-white flex flex-col z-10 shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-[#fdfdfd]">
            {tasks.length === 0 && (
              <p className="text-xs text-slate-400 italic text-center mt-4">No active tasks</p>
            )}
            {tasks.map((task) => {
              ...
              return (
                <div
                  key={task.id}
                  className="bg-white rounded-sm p-3 border border-slate-200 shadow-sm animate-in fade-in slide-in-from-left-2 transition-all hover:border-slate-300"
                >
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    {assignee ? (
                      <div
                        className="flex items-center gap-1 bg-slate-50 px-2 py-0.5 border border-slate-200 font-mono text-[10px]"
                      ...
                      <div className="text-[10px] font-mono text-slate-400 bg-slate-50 px-2 py-0.5 border border-slate-200">
                        Unassigned
                      </div>
                    )}
                  </div>
        ...
        <div className="flex-1 relative bg-[#f8fafc]">
          ...
          >
            ...
            {layoutedAgents.find((a) => a.id === "user") &&
              layoutedAgents.find((a) => a.id === "leader") && (
                <path
                  d={`M 50 12 L 50 35`}
                  fill="none"
                  stroke={agents.user.status === "working" ? agents.user.color : "#e2e8f0"}
          ...
          {layoutedAgents.map((agent) => {
            ...
            return (
              ...
              >
                ...
                {activeTask && (
                  <div
                    className="absolute -top-10 whitespace-nowrap bg-white text-slate-700 text-[10px] px-3 py-1.5 border shadow-sm animate-in slide-in-from-bottom-2 fade-in z-20 flex items-center gap-1.5"
                ...
                >
                  ...
                  {isError && (
                    <div className="absolute -bottom-1 -right-1 bg-red-500 text-white p-0.5 border-2 border-white">
            ...
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-50 bg-white border border-amber-300 p-6 shadow-2xl w-[400px] animate-in zoom-in-95 duration-200">
              ...
              <div className="bg-amber-50/50 p-3 mb-5 border border-amber-100">
                <p className="text-[10px] font-mono text-slate-500 mb-1 uppercase tracking-wider">
                  Source: <span className="text-blue-600 font-bold">{interaction.agentName}</span>
                </p>
              ...
              <div className="space-y-2">
                {interaction.options.map((opt) => (
                  ...
                  >
                    ...
                    <span className="text-amber-600 text-[10px] uppercase font-bold tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">
                      Select
                    </span>
                  </button>
                ))}
        ...
        <div className="w-[340px] border-l border-slate-200 bg-white flex flex-col z-10 shadow-sm">
          ...
          <div className="flex-1 p-6 flex flex-col items-center justify-start gap-4 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] bg-opacity-5 overflow-y-auto pt-6">
            {/* 1. Agent Config Yaml */}
            <div
              className={`w-full max-w-[260px] h-12 shrink-0 flex items-center px-4 transition-all duration-700 relative ${artifactState.agentConfig ? "border border-blue-500 bg-blue-50 artifact-part-reveal shadow-sm" : "border border-dashed border-slate-300 bg-transparent"}`}
            >
              <FileJson
                size={16}
                className={artifactState.agentConfig ? "text-blue-500 mr-3" : "text-slate-400 mr-3"}
              />
              <span
                className={`text-[10px] uppercase tracking-widest font-bold ${artifactState.agentConfig ? "text-blue-600" : "text-slate-400"}`}
              >
            ...
            {/* 2. System Prompt */}
            <div
              className={`w-full max-w-[260px] h-12 shrink-0 flex items-center px-4 transition-all duration-700 relative ${artifactState.systemPrompt ? "border border-purple-500 bg-purple-50 artifact-part-reveal shadow-sm" : "border border-dashed border-slate-300 bg-transparent"}`}
            >
              <Terminal
                size={16}
                className={
                  artifactState.systemPrompt ? "text-purple-500 mr-3" : "text-slate-400 mr-3"
                }
              />
              <span
                className={`text-[10px] uppercase tracking-widest font-bold ${artifactState.systemPrompt ? "text-purple-600" : "text-slate-400"}`}
              >
              ...
              <div className="flex gap-2 flex-wrap justify-start">
                {Array.from({ length: builderCount }).map((_, i) => {
                  ...
                  return (
                    <div
                      key={id}
                      className={`flex-1 min-w-[70px] h-14 flex flex-col items-center justify-center transition-all duration-700 ${isDone ? "border border-amber-500 bg-amber-50 artifact-part-reveal shadow-sm" : "border border-dashed border-slate-300 bg-transparent"}`}
                    >
                      <Zap
                        size={14}
                        className={isDone ? "text-amber-500 mb-1.5" : "text-slate-300 mb-1.5"}
                      />
                      <span
                        className={`text-[9px] font-bold text-center uppercase tracking-wider px-1 ${isDone ? "text-amber-600" : "text-slate-400"}`}
                      >
                        Skill 0${i + 1}
                      </span>
                    </div>
            ...
            {/* 4. Tool Config */}
            <div
              className={`w-full max-w-[260px] h-12 shrink-0 flex items-center px-4 transition-all duration-700 relative ${artifactState.toolConfig ? "border border-emerald-500 bg-emerald-50 artifact-part-reveal shadow-sm" : "border border-dashed border-slate-300 bg-transparent"}`}
            >
              <Wrench
                size={16}
                className={
                  artifactState.toolConfig ? "text-emerald-500 mr-3" : "text-slate-400 mr-3"
                }
              />
              <span
                className={`text-[10px] uppercase tracking-widest font-bold ${artifactState.toolConfig ? "text-emerald-600" : "text-slate-400"}`}
              >
            ...
            <div
              className={`w-full max-w-[260px] h-12 shrink-0 flex items-center px-4 transition-all duration-700 relative ${artifactState.startupScript ? "border border-indigo-500 bg-indigo-50 artifact-part-reveal shadow-sm" : "border border-dashed border-slate-300 bg-transparent"}`}
            >
              <Rocket
                size={16}
                className={
                  artifactState.startupScript ? "text-indigo-500 mr-3" : "text-slate-400 mr-3"
                }
              />
              <span
                className={`text-[10px] uppercase tracking-widest font-bold ${artifactState.startupScript ? "text-indigo-600" : "text-slate-400"}`}
              >
          ...
          <div className="h-36 border-t border-slate-200 bg-white flex flex-col shrink-0">
            <div className="p-2.5 border-b border-slate-100 flex items-center gap-2 bg-slate-50">
              <ShieldCheck
                size={14}
                className={
                  e2eStats.status === "running"
                    ? "text-blue-500 animate-pulse"
                    : e2eStats.status === "failed"
                      ? "text-red-500"
                      : e2eStats.status === "passed"
                        ? "text-emerald-500"
                        : "text-slate-400"
                }
              />
              <h2 className="font-semibold text-[11px] tracking-wider uppercase text-slate-700">
                E2E Verification
              </h2>
              {e2eStats.status === "running" && (
                <span className="ml-auto text-[9px] text-blue-500 font-bold animate-pulse uppercase">
                  Running...
                </span>
              )}
              {e2eStats.status === "failed" && (
                <span className="ml-auto text-[9px] text-red-500 font-bold uppercase">Failed</span>
              )}
              {e2eStats.status === "passed" && (
                <span className="ml-auto text-[9px] text-emerald-500 font-bold uppercase">
                  Passed
                </span>
              )}
            </div>
            <div className="flex-1 p-4 flex flex-col justify-center">
              <div className="flex justify-between items-end mb-2">
                ...
                <div className="font-mono font-bold text-lg">
                  <span
                    className={`transition-colors duration-300 ${e2eStats.status === "failed" ? "text-red-500" : e2eStats.status === "passed" ? "text-emerald-500" : "text-blue-600"}`}
                  >
                    {e2eStats.passed}
                  </span>
              ...
              <div className="mt-2 text-[9px] font-mono text-slate-400 uppercase tracking-widest overflow-hidden text-ellipsis whitespace-nowrap">
                {e2eStats.status === "idle"
                  ? "> Waiting for deployment..."
                  : e2eStats.status === "running"
                    ? "> Executing suite [auth, api, core]..."
                    : e2eStats.status === "failed"
                      ? "> Error at node_modules. Halting test."
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-[#fdfdfd]">
          {messages.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`p-2.5 px-3 shadow-sm text-sm border transition-colors animate-in fade-in slide-in-from-bottom-2 ${bgClass}`}
              >
                <div className="flex items-baseline gap-2 font-mono">
                  ...
                  {!isSystem && msg.toId && agents[msg.toId] && (
                    <span className="text-[10px] text-slate-400">
                      ▶{" "}
                      <span style={{ color: agents[msg.toId].color }}>
                        @{agents[msg.toId].name}
                      </span>
                    </span>
                  )}
