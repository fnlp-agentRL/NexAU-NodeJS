...
function agentVisuals(
  ...
  if (r.includes("test") || r.includes("qa")) return { icon: ShieldCheck, color: "#ea580c" };
  ...
  for (let i = agent.blocks.length - 1; i >= 0; i--) {
    ...
    if (b.kind === "thinking")
      return agent.isActive
        ? { label: "思考中", icon: Brain, color: "#8b5cf6" }
        : { label: "已完成", icon: CheckCircle2, color: "#10b981" };
    if (b.kind === "tool_call") {
      if (!agent.isActive) return { label: "已完成", icon: CheckCircle2, color: "#10b981" };
      const n = b.name.toLowerCase();
      if (n === "ask_user") return { label: "对话中", icon: MessageCircle, color: "#059669" };
    ...
    if (b.kind === "text")
      return agent.isActive
        ? { label: "对话中", icon: MessageCircle, color: "#059669" }
        : { label: "已完成", icon: CheckCircle2, color: "#10b981" };
...
export default function App() {
  ...
  const currentMockTasks =
    IS_MOCK_MODE && "mockTasks" in stream ? (stream as typeof mockStream).mockTasks : null;
  ...
  const layoutNodes: LayoutNode[] = useMemo(() => {
    ...
    const firstId = leaderAgent
      ? leaderAgent.agentId
      : list.length > 0
        ? list.sort((a, b) => a.agentId.localeCompare(b.agentId))[0].agentId
        : null;
    ...
    workers.forEach((w, i) => {
      ...
      const x = workers.length === 1 ? 50 : 15 + (70 / (workers.length - 1)) * i;
      nodes.push({
        id: w.agentId,
        role: w.roleName || "Worker",
        name: w.agentId,
        x,
        y: 75,
        isActive: w.isActive,
  ...
  const agentTodoLabel = useMemo(() => {
    ...
    for (const agent of Object.values(agents)) {
      ...
      for (let i = agent.blocks.length - 1; i >= 0; i--) {
        ...
        try {
          const parsed = JSON.parse(b.args);
          const todos: { description?: string; status?: string }[] =
            parsed.todos ?? parsed.items ?? [];
          const inProgress = todos.find((t) => t.status === "in_progress");
          if (inProgress?.description) {
            label = inProgress.description;
            break;
          }
      ...
      for (let i = 0; i < agent.blocks.length; i++) {
        ...
        if (b.kind === "user_message") {
          ...
          msgs.push({
            ...
            fromName: senderName,
            toId: agent.agentId,
            toName: resolveAgentName(agent.agentId),
            text: b.content,
            type: msgType,
          });
          ...
          msgs.push({
            ...
            fromId: b.from,
            fromName: resolveAgentName(b.from),
            toId: agent.agentId,
            toName: resolveAgentName(agent.agentId),
            text: b.content,
            type: "team",
  ...
  // --- Selected agent ---
  const selectedAgent: AgentState | null = selectedAgentId
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  {!node.isActive &&
                    !node.isVirtual &&
                    agents[node.id]?.blocks.length > 0 &&
                    !(activeAsk?.hidden && activeAsk.agentId === node.id) && (
                ...
                <div
                  className={`mt-2 text-center bg-white px-2 py-1 border shadow-sm ${isSelected ? "border-slate-400" : "border-slate-200"}`}
                >
                  <p
                  ...
                  <p className="text-[10px] font-mono text-slate-500 truncate max-w-[80px]">
                    {node.name}
                  </p>
                </div>
                {/* Task progress bar */}
                {!node.isVirtual &&
                  agentTaskProgress[node.id] &&
                  (() => {
                      ...
                      <div className="mt-1 flex flex-col items-center w-20">
                        <div className="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${pct}%`,
                              backgroundColor: pct === 100 ? "#10b981" : node.color,
                            }}
                          />
                        </div>
                ...
                {!node.isVirtual &&
                  (agentTodoLabel[node.id] || agentActivities[node.id]) &&
                  (() => {
        ...
        <div className="h-56 border-t border-slate-200 bg-white shrink-0 flex flex-col">
            ...
            {streamMessages.map((msg) => {
              ...
              return (
                <div key={msg.id} className={`p-2 px-3 text-sm border shadow-sm ${bgClass}`}>
                  <span
                    className="font-bold text-[11px] uppercase tracking-wider mr-2"
                    style={{ color: msg.type === "user" ? "#059669" : "#6366f1" }}
                  >
                    {msg.fromName}
                  </span>
                  {msg.toName && (
          ...
          <div className="h-12 border-t border-slate-200 flex items-center px-4 gap-2 bg-white shrink-0">
            ...
            <input
              ...
              placeholder={
                isStreaming ? "Send message to agent..." : "Type a message to start the team..."
              }
                ...
                <div className="p-3 border-b border-slate-200 flex items-center justify-between bg-slate-50">
                  <div className="flex items-center gap-2">
                    ...
                    {selectedAgent.isActive && (
                      <span className="text-[9px] text-blue-500 font-bold animate-pulse uppercase">
                        Active
                      </span>
                    )}
                  </div>
          ...
          <div className="bg-white w-[70vw] h-[80vh] flex flex-col shadow-xl rounded-lg overflow-hidden">
            ...
            <div className="flex-1 overflow-hidden">
              <FileViewer
                ...
                mockFiles={IS_MOCK_MODE ? MOCK_FILE_CONTENTS : undefined}
              />
            </div>
          </div>
        </div>
